...
DATABRICKS_BASE_URL = "https://dbc-d7920208-a62f.cloud.databricks.com"
DATABRICKS_TOKEN = "dapifa21789da1af73881af0d4159052ec41"
DATABRICKS_ENDPOINT = "quickstart-agent-endpoint"

OCR_MASTER_PROMPT = "Base on this Context, answer the Question and briefly respond with an answer without detailed explanation unless asked. "

OCR_KEYWORDS = {
    # Original
    "convert this text", "translate this text", "solve this problem", 
    "how do i answer this", "help answer this question",
    
    # Extraction / Reading
    "read this", "what does this say", "extract the text", "pull out the words", 
    "get the writing", "read the words",
    
    # Specific Documents / Contexts
    "invoice", "receipt", "form", "document", "handwriting", "read the sign",
    
    # Data Processing / Intent
    "calculate", "summarize this", "make this editable", "digitize this", 
    "answer the question on this", "what is the date on this", "what is the amount"
}


VISUAL_KEYWORDS = {
    # Original
    "describe this image", "what is this photo", "what do you see", 
    "tell me about this image", "what animal is this", "describe the image", 
    "describe this pic", "what is in this picture",
    
    # General Description
    "show me the", "what kind of", "what is the color", "identify this", "analyze this image",
    
    # Content / Objects
    "what is this object", "what type of car", "what kind of dog", "what flower is this", 
    "who is this person", "what building is this",
    
    # Scene / Setting
    "where is this", "what time of day", "what is the background", "what is the weather like",
    
    # Actions / Events
    "what is happening", "what are they doing", "action in this image", "what event is this",
    
    # Subjective / Style
    "how would you describe the style", "what is the mood", "is this vintage", "is this drawn"
}
...
try:
    ...
    # Handle case where easyocr fails to initialize (e.g., missing dependencies)
    print(f"⚠️ Warning: Failed to initialize easyocr. OCR functionality may be broken. Error: {e}")
    reader = None


def check_for_keywords(text: str, keywords: set) -> bool:
    """Checks if the lowercase text contains any of the keywords."""
    text_lower = text.lower()
    return any(keyword in text_lower for keyword in keywords)


def decode_and_ocr_image(base64_image: str) -> str:
    """
    Decodes the base64 string, opens the image, and performs OCR 
    using the easyocr library.
    """
    if reader is None:
        return "❌ OCR Error: easyocr engine failed to initialize during startup."
        
    try:
        # 1. Clean the string of all potential headers/labels
        base64_data = base64_image.strip()

        # Remove common web header prefix (data:image/xyz;base64,)
        if ',' in base64_data:
            base64_data = base64_data.split(',', 1)[-1]
        
        # Remove the custom 'Base 64 Data: ' label
        base64_data = base64_data.replace('Base 64 Data:', '').strip() 

        # 2. Convert the clean Base64 string into an encoded bytes object
        encoded_bytes = base64_data.encode('ascii')

        # 3. Decode the raw base64 bytes to raw image data
        img_data = base64.b64decode(encoded_bytes)
        
        # 4. Perform OCR using easyocr directly on the image bytes
        # easyocr can take the bytes as input
        
        # The reader returns a list of tuples: (bbox, text, confidence)
        results = reader.readtext(img_data)
        
        # Extract only the text component and join them into a single string
        # We join with a newline for better readability/separation of recognized lines
        ocr_text = "\n".join([text for (bbox, text, conf) in results])
        
        print(f"\n\nOCR Text (easyocr): {ocr_text}")
...
def decode_and_ocr_imageX(base64_image: str) -> str:
    """
    Decodes the base64 string, opens the image, and performs OCR 
    using Tesseract.
    """
    try:
        # 1. Strip the header ("data:image/xyz;base64,") if it exists

        base64_data = base64_image.strip()

        if ',' in base64_data:
            # We assume the actual base64 data is everything after the first comma
            base64_data = base64_data.split(',', 1)[-1]


        print(f"\n\nBase 64 Data: {base64_data}")

        encoded_bytes = base64_data.encode('ascii')

        # 1. Decode the base64 string to raw image data
        #img_data = base64.b64decode(base64_image)
        img_data = base64.b64decode(encoded_bytes)
        
        # 2. Open the image using BytesIO and PIL
...
async def health_assessment_flow(chat_history):
    ...
    if current_question_index >= len(questions):
        ...
        try:
             ...
             return await call_databricks_model(text=prompt)
...
modelHealth.load_state_dict(torch.load("cardio_modelX.pt"))
modelHealth.eval()
...
def predict_stroke_risk(user_input: dict) -> tuple:
    ...
    try:
        ...
        #scaler = joblib.load("scaler.save")
        scaler = joblib.load("scaler_health_strokeX.save")
...
import ollama
#from openai import OpenAI # Assuming you use OpenAI client for Databricks LLM API
import os
from typing import Optional
import requests

# --- Databricks Model Function (Needs Implementation) ---

from typing import Optional

# --- Databricks Model Function (Updated with requests logic) ---

async def call_databricks_model(text: str, image_base64: Optional[str] = None, model: Optional[str] = None) -> str:
    """
    Calls the Databricks LLM endpoint using the requests library.
    This function uses synchronous 'requests.post' inside an 'async def', 
    which is generally discouraged in production for blocking event loops.
    """
    # 1. Check for image (Databricks custom endpoints often don't support it directly)
    #if image_base64:
    #    return "❌ Error: Databricks custom endpoint call attempted with image_base64. Multimodal handling not implemented."

    # 2. Get credentials from environment variables
    DB_BASE_URL = DATABRICKS_BASE_URL
    DB_TOKEN = DATABRICKS_TOKEN
    DB_ENDPOINT = DATABRICKS_ENDPOINT
    
    if not DB_BASE_URL or not DB_TOKEN:
         return "❌ Error: Databricks credentials (DB_TOKEN/DB_BASE_URL) not found in environment."

    try:
        # 3. Define endpoint and payload
        endpoint_name = DB_ENDPOINT  
        endpoint_url = f"{DB_BASE_URL}/serving-endpoints/{endpoint_name}/invocations"
        print(f"\nDatabricks Endpoint URL: {endpoint_url}")
        ...
        payload = {
            ...
            # Context and custom inputs are often optional but included as in your example
            "context": {}, 
            "custom_inputs": {}
        }
        ...
        print(f"\nDatabricks Response Status Code: {response.status_code}")

        # 5. Handle the response
        if response.status_code == 200:
            try:
                response_json = response.json()
                # Assuming the response format follows the standard chat response structure
                # You might need to adjust the path based on your specific endpoint's output
                #response_text = response_json.get("choices", [{}])[0].get("message", {}).get("content", "[No response content]")
                response_text = response_json.get("messages", [{}])[0].get("content", "[No response content]")
                # Check for an alternative structure if the above fails (e.g., custom agent response)
                if response_text == "[No response content]":
                    # For custom endpoints, the output structure is highly variable.
                    # We'll just return the full JSON response if we can't parse the message content.
                    return f"{response_text}"
                    
                return response_text.strip()
            except json.JSONDecodeError:
                return f"❌ Error: Databricks call returned non-JSON response: {response.text}"
            except Exception as e:
                # Catching issues with parsing the JSON structure
                return f"❌ Error parsing Databricks JSON response: {str(e)}"
        else:
            try:
                # Print and return error details if status code is not 200
                error_details = response.json()
                print(f"Databricks Error Details: {error_details}")
                return f"❌ Databricks API Error ({response.status_code}): {error_details.get('message', 'Unknown Error')}"
            except json.JSONDecodeError:
                return f"❌ Databricks API Error ({response.status_code}): {response.text}"


    except Exception as e:
        return f"❌ Error calling Databricks LLM: {str(e)}"
...
async def call_ollama_model(text: str, image_base64: Optional[str] = None, 
                            model: str = "gemma3:4b") -> str:
    """
    Branches based on question type: Visual, OCR/Text Processing, or Databricks.
    """
    print("\nUser Prompt:", text)
    
    # 1. Check for Databricks first (highest priority)
    if "databricks" in text.lower():
        print("\n--- Conditional Triggered: Calling Databricks LLM ---")
        # Assuming you call a Databricks function here
        # return await call_databricks_model(text, image_base64)
        #return await call_ollama_model2(text=text, image_base64=image_base64, model="gemma3:4b")

        return await call_databricks_model(text, image_base64, model=model)
    
    # 2. Check for Visual Description Keywords (Requires an image)
    elif image_base64 and check_for_keywords(text, VISUAL_KEYWORDS):
        print("\n--- Conditional Triggered: Visual Description (Ollama Gemma3n) ---")
        # Call the specific visual model (Gemma3n) with the image
        try:
            return await call_ollama_model2(text=text, image_base64=image_base64, model="gemma3:4b")
            #client = ollama.Client()
            #kwargs = {
            #    "model": "gemma3:4b",  # Your specific visual model
            #    "prompt": text,
            #    "images": [image_base64]
            #}
            #response = client.generate(**kwargs)
            return response.get("response", "[No response from Gemma3n]").strip()
        except Exception as e:
            return f"❌ Error calling Ollama Gemma3n: {str(e)}"
    
    # 3. Check for OCR/Text Processing Keywords (Implies text extraction followed by LLM)
    elif image_base64:   # and check_for_keywords(text, OCR_KEYWORDS):
        print("\n--- Conditional Triggered: Calling Databricks - OCR/Text Processing (OCR Step) ---")
        
        # NOTE: You need a tool/model here to perform the OCR (Image -> Text)
        # Then, send the extracted text to your default LLM (Gemma3:4b) or another model.
        
        # Placeholder for the OCR process:
        ocr_extracted_text = decode_and_ocr_image(image_base64)
	#"The quick brown fox jumps over the lazy dog." # Replace with actual OCR call
        
        new_prompt = f"Using the following extracted text: '{ocr_extracted_text}', {text}"
         
        
        try:
            OCR_PROMPT = f"{OCR_MASTER_PROMPT} Question: {text} Context: {new_prompt}"
            print(f"\n\n{OCR_PROMPT}")
            return await call_databricks_model(OCR_PROMPT, None, model=model)
        except Exception as e:
            return f"❌ Error calling Databricks for OCR follow-up: {str(e)}"
    
    # 4. Default Case (Standard text prompt, no image, or unknown keywords)
    else:
        print("\n--- Default Case: Calling Databricks (Standard) ---")
        try:
            return await call_databricks_model(text, image_base64, model=model)
            #return await call_ollama_model2(text=text, image_base64=image_base64, model="gemma3:4b")
...
async def call_ollama_model3(text: str, image_base64: Optional[str] = None, 
                            model: str = "gemma3:4b") -> str:
    """
    Conditionally calls the Databricks LLM or the local Ollama model.
    """
    print("\n\n", text)
    
    # 1. Check for the keyword "databricks" (case-insensitive)
    if "databricks" in model.lower():
        print("\n\nCalling Databricks LLM...")
        # 2. Call the Databricks function
        # You can choose a specific Databricks model here, e.g., 'databricks-dbrx-instruct'
        return await call_databricks_model(text, image_base64, model=model)
    
    # 3. Else, proceed with the original Ollama logic
    try:
        print("\n\nCalling Ollama...")
        client = ollama.Client()
...
async def call_ollama_model2(text: str, image_base64: str = None, model: str = "gemma3:4b") -> str:
    ...
    try:
        ...
        if image_base64:
            ...
            #print(f"\n\n\n{image_base64}\n\n\n")
            #kwargs["images"] = f"data:image/gif;base64,{image_base64}" # list of base64 strings
...
async def server_loop(q):
    ...
    while True:
        try:
            ...
            print(f"\n🕐 [{datetime.datetime.now()}] Calling LLM with model='{model}'")
            ...
            if "I like to get a health assessment".lower() in text.lower() and "Thank you for submitting the assessment".lower() in text.lower():
                text = f"{MASTER_PROMPT_HEALTH_MORE_QUESTIONS} {text}"

            if "I like to get a health assessment".lower() in text.lower() and "Thank you for submitting the assessment".lower() not in text.lower():
